...
#define CM_BLOCK_BLANK	9085
#define CM_BLOCK_THRUSTER	9086
...
int fps=0,frame=0,maxfps=0;
long block_step_count=0,block_executed=0;
...
const byte BBLANK=12;
const byte BTHRUSTER=13;
...
void init_block_types()
{
  ...
  block_types[12][0]=BBLANK; // blank
  block_types[12][1]=11; //sprite index
  
  block_types[13][0]=BTHRUSTER; // thruster
  block_types[13][1]=12; //sprite index
...
void thruster_input(block *lnode)
{
  lnode->inputs=0;
...
void thruster_exec(block *lnode)
{
  /*
  switch(lnode->direction)
  {
...
void thruster_step(block *lnode)
{
  ...
  if (nnn!=NULL)
  {
    ...
    if (lnode->input==1)
    {
      ...
      //nnn=grid[xx][yy];
        ...
        {
        openlist_push(lnode);
        lnode->movedir=(lnode->direction+2)%4;
        lnode->instack=1;   
        nnn->func1_routine(nnn);  //comment out this line for none continuse movement                
        }
    }
  }  
...
void thruster_draw(block *lnode)
{
  draw_bitmap(block_sprite[lnode->sprite_index],lnode->x-view_left,lnode->y-view_top,lnode->direction);
}
...
void blank_input(block *lnode)
{
  lnode->inputs=0;
        
}

void blank_exec(block *lnode)
{

}

void blank_step(block *lnode)
{

}


void blank_draw(block *lnode)
{
  int x,y;
  x=lnode->x-view_left;
  y=lnode->y-view_top;
  ...
  draw_bitmap(block_sprite[lnode->sprite_index],lnode->x-view_left,lnode->y-view_top,0);
...
HBITMAP tb_edit;
HBITMAP tb_edit_mask;

HBITMAP tb_blank;
HBITMAP tb_blank_mask;

HBITMAP tb_thruster;
HBITMAP tb_thruster_mask;
...
void block_sprite_load(HWND hwnd)
{
  ...
  id=11;
  block_sprite[id]=(HBITMAP)LoadImage(NULL, "blank.bmp", IMAGE_BITMAP, 0, 0,LR_LOADFROMFILE);
  if (block_sprite[id] == NULL)
    MessageBox(hwnd, "blank Could not load bitmap!", "Error", MB_OK | MB_ICONEXCLAMATION);
    
  id=12;
  block_sprite[id]=(HBITMAP)LoadImage(NULL, "thruster.bmp", IMAGE_BITMAP, 0, 0,LR_LOADFROMFILE);
  if (block_sprite[id] == NULL)
    MessageBox(hwnd, "thruster Could not load bitmap!", "Error", MB_OK | MB_ICONEXCLAMATION);
  ...
  tb_edit=(HBITMAP)LoadImage(NULL, "tb_edit.bmp", IMAGE_BITMAP, 0, 0,LR_LOADFROMFILE);
  tb_edit_mask=CreateBitmapMask(tb_edit, RGB(255,0,255));
  
  tb_blank=(HBITMAP)LoadImage(NULL, "tb_blank.bmp", IMAGE_BITMAP, 0, 0,LR_LOADFROMFILE);
  tb_blank_mask=CreateBitmapMask(tb_blank, RGB(255,0,255));
  
  tb_thruster=(HBITMAP)LoadImage(NULL, "tb_thruster.bmp", IMAGE_BITMAP, 0, 0,LR_LOADFROMFILE);
  tb_thruster_mask=CreateBitmapMask(tb_thruster, RGB(255,0,255));
...

{
    ...
    hwnd = CreateWindowEx (
           ...
           "Self Replcating Machine",       /* Title Text */
...
struct block* block_create(int xx,int yy,byte blocktype)
{
 struct block *lnode;
  ...
  if (blocktype==BBLANK)
  {
    lnode->draw_routine=blank_draw;   
    lnode->func0_routine=blank_input;    
    lnode->func1_routine=blank_exec;   
    lnode->func2_routine=blank_step;                  
  }
  
  if (blocktype==BTHRUSTER)
  {
    lnode->draw_routine=thruster_draw;   
    lnode->func0_routine=thruster_input;    
    lnode->func1_routine=thruster_exec;   
    lnode->func2_routine=thruster_step;                  
  }
...
struct copyblock *copyblockhead, *copyblocktail;

void append_copyblock(struct copyblock *lnode) {
 if(copyblockhead == NULL) {
  copyblockhead = lnode;
  lnode->prev = NULL;
 } else {
  copyblocktail->next = lnode;
  lnode->prev = copyblocktail;
 }
...
void remove_copyblock(struct copyblock *lnode) {
 if(lnode->prev == NULL)
  copyblockhead = lnode->next;
 else
  lnode->prev->next = lnode->next;

 if(lnode->next == NULL)
  copyblocktail = lnode->prev;
 else
  lnode->next->prev = lnode->prev;
}


void get_copy_blocks(int x,int y,int xx, int yy)
{
     
  block *next, *now;  
  struct copyblock *lnode;
  ...
  if (now!=NULL)
  {
    while(1==1)
    {
      ...
      if (next==NULL)
      {
        if ((now->xx>=x) && (now->xx<xx) && (now->yy>=y) && (now->yy<yy))
        {
          lnode = (struct copyblock *)malloc(sizeof(struct copyblock));
          append_copyblock(lnode);
          lnode->xx=now->xx-x;
          lnode->yy=now->yy-y;
          lnode->dir=now->direction;
          lnode->type=now->blocktype;  
          lnode->inputs=now->inputs;
          lnode->outputs=now->outputs; 
          lnode->connection[0]=now->connection[0];
          lnode->connection[1]=now->connection[1];
          lnode->connection[2]=now->connection[2];
          lnode->connection[3]=now->connection[3];
        }
        break;            
      }
...
void paste_copy_blocks(int x,int y)
{
  ...
  if (now!=NULL)
  {
    while(1==1)
    {
      ...
      if (next==NULL)
      {
        ...
        selecteddirection=now->dir;
        grid[now->xx+x][now->yy+y]=block_create(now->xx+x,now->yy+y,now->type);
        data=grid[now->xx+x][now->yy+y];
        data->outputs=now->outputs;
        data->inputs=now->inputs;
        data->connection[0]=now->connection[0];
        data->connection[1]=now->connection[1];
        data->connection[2]=now->connection[2];
        data->connection[3]=now->connection[3];
        
        //grid[now->xx+x][now->yy+y]->func0_routine(grid[now->xx+x][now->yy+y]);
        //grid[now->xx+x][now->yy+y]->func2_routine(grid[now->xx+x][now->yy+y]);
...
struct selectblock *selectblockhead, *selectblocktail;

void append_selectblock(struct selectblock *lnode) {
 if(selectblockhead == NULL) {
  selectblockhead = lnode;
  lnode->prev = NULL;
 } else {
  selectblocktail->next = lnode;
  lnode->prev = selectblocktail;
 }

 selectblocktail = lnode;
 lnode->next = NULL;
}



void remove_selectblock(struct selectblock *lnode) {
 if(lnode->prev == NULL)
  selectblockhead = lnode->next;
 else
  lnode->prev->next = lnode->next;

 if(lnode->next == NULL)
  selectblocktail = lnode->prev;
 else
  lnode->next->prev = lnode->prev;
}
...
void get_select_blocks(int x,int y,int xx, int yy)
{
     
  block *next, *now;
  
  struct selectblock *lnode;
  ...
  if (now!=NULL)
  {
    while(1==1)
    {
      ...
      if (next==NULL)
      {
        if ((now->xx>=x) && (now->xx<xx) && (now->yy>=y) && (now->yy<yy))
        {
          lnode = (struct selectblock *)malloc(sizeof(struct selectblock));
          append_selectblock(lnode);
          lnode->data=now;
          lnode->posx=now->xx;
          lnode->posy=now->yy;
...
void drag_select_blocks(int x,int y,int xx, int yy)
{
  ...
  if (now!=NULL)
  {
    while(1==1)
    {
      ...
      if (next==NULL)
      {
        //grid[now->data->xx][now->data->yy]=NULL;     
        now->data->x = ((now->posx*32)+(-(x-xx)*32));
        now->data->y = ((now->posy*32)+(-(y-yy)*32));
        
        now->data->xx = ((now->posx)+(-(x-xx)));
        now->data->yy = ((now->posy)+(-(y-yy)));
        
        //grid[now->data->xx][now->data->yy]=now->data;
...
void move_select_blocks()
{
  ...
  if (now!=NULL)
  {
    while(1==1)
    {
      ...
      if (next==NULL)
      {
        grid[now->posx][now->posy]=NULL;
        grid[now->data->xx][now->data->yy]=now->data;
        //free(now);
        //free(selectblockhead);
        //selectblockhead=NULL;
        break;            
      }
...
block *mouseblock;
int mousetx=0,mousety=0;
int mouse_dragx=0,mouse_dragy=0,mouse_endx=0,mouse_endy=0;
int mouse_selected=0;
...
void mouse_event(byte event)
{
  ...
  if (event==LeftDown)
  {
    ...
    else if (selectedmode==3)
    {
       ...
       else if (mouse_selected==2)
       {
       
         
         if ((mouse_dragx<mouse_endx) && (mouse_dragy<mouse_endy))
         {
           if ((xx>=mouse_dragx) && (xx<mouse_endx) && (yy>=mouse_dragy) && (yy<mouse_endy))
           {
             mouse_selected=3; 
             //get_select_blocks(mouse_dragx,mouse_dragy,mouse_endx,mouse_endy);
             mouse_dragx=xx;   
             mouse_dragy=yy;                 
           }
           else
         ...
         if ((mouse_dragx>mouse_endx) && (mouse_dragy<mouse_endy))
         {
           if ((xx>=mouse_endx) && (xx<mouse_dragx) && (yy>=mouse_dragy) && (yy<mouse_endy))
           {
             mouse_selected=3;
             //get_select_blocks(mouse_endx,mouse_dragy,mouse_dragx,mouse_endy);
             mouse_dragx=xx;   
             mouse_dragy=yy;                 
           }
           else
         ...
         if ((mouse_dragx>mouse_endx) && (mouse_dragy>mouse_endy))
         {
           if ((xx>=mouse_endx) && (xx<mouse_dragx) && (yy>=mouse_endy) && (yy<mouse_dragy))
           {
             mouse_selected=3; 
             //get_select_blocks(mouse_endx,mouse_endy,mouse_dragx,mouse_dragy);
             mouse_dragx=xx;   
             mouse_dragy=yy;                    
           }
           else
         ...
         if ((mouse_dragx<mouse_endx) && (mouse_dragy>mouse_endy))
         {
           if ((xx>=mouse_dragx) && (xx<mouse_endx) && (yy>=mouse_endy) && (yy<mouse_dragy))
           {
             mouse_selected=3;  
             //get_select_blocks(mouse_dragx,mouse_endy,mouse_endx,mouse_dragy);
             mouse_dragx=xx;   
             mouse_dragy=yy;                  
           }
           else
  ...
  if (event==LeftUp)
  {
    if (selectedmode==3)
    {
       if (mouse_selected==1)
       {
         if ((mouse_dragx==xx) && (mouse_dragy==yy))
         {
           mouse_selected=0;
         }
         else
  ...
  if (event==MouseMove)
  {
    if (selectedmode==3)
    {
       ...
       {
         drag_select_blocks(mouse_dragx,mouse_dragy,xx,yy);      
       }
    }                    
  }
...
void keyboard_event()
{
  ...
  if ((check_key(VK_DELETE)==1) && (selectedmode==3) && (mouse_selected==2))
  {
    delete_select_blocks();
    mouse_selected=0;                    
  }
  
  if (check_key(86)==1)//V
  {
    paste_copy_blocks((int)floor((mousex+view_left)/32),(int)floor((mousey+view_top)/32));                    
  }
  
  if ((check_key(67)==1) && (selectedmode==3) && (mouse_selected==2))//C
  {
    free_copy_blocks();
    
    if ((mouse_dragx<mouse_endx) && (mouse_dragy<mouse_endy))
    {
      get_copy_blocks(mouse_dragx,mouse_dragy,mouse_endx,mouse_endy);                
    }
         
    if ((mouse_dragx>mouse_endx) && (mouse_dragy<mouse_endy))
    {
      get_copy_blocks(mouse_endx,mouse_dragy,mouse_dragx,mouse_endy);
    }
         
    if ((mouse_dragx>mouse_endx) && (mouse_dragy>mouse_endy))
    {
      get_copy_blocks(mouse_endx,mouse_endy,mouse_dragx,mouse_dragy);
    }
         
    if ((mouse_dragx<mouse_endx) && (mouse_dragy>mouse_endy))
    { 
      get_copy_blocks(mouse_dragx,mouse_endy,mouse_endx,mouse_dragy);
    }
  }
...
void new_grid()
{
  ...
  SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_BLANK ,  MAKELONG(0, 0) );
  SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_THRUSTER ,  MAKELONG(0, 0) );
  ...
  SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_EDIT ,  MAKELONG(0, 0) );
...
void open_grid()
{
  ...
  if (GetFileName( MainWindow, szFileName, FALSE)==0)
  {
    return;
  }
  ...
  while (1==1)
  {
    ...
    type=chr;
    xx=(fgetc(fn)*256)+fgetc(fn);
    yy=(fgetc(fn)*256)+fgetc(fn);
    dir=fgetc(fn);
    ...
    now=grid[xx][yy];
    now->outputs=fgetc(fn);
    now->inputs=fgetc(fn);
    now->connection[0]=fgetc(fn);
    now->connection[1]=fgetc(fn);
    now->connection[2]=fgetc(fn);
    now->connection[3]=fgetc(fn);
    
    now->func0_routine(now);          
    //now->func2_routine(now);
    
    if ((type==BNOT)||(type==BNOR))
    {                 
...
void save_grid()
{
  ...
  if (GetFileName( MainWindow, szFileName, TRUE)==0)
  {
    return;
  }
  
  fn=fopen ( szFileName, "wb");
  ...
  if (now!=NULL)
  {
    while(1==1)
    {
      ...
      if (next==NULL)
      {
        ...
        fputc(now->blocktype,fn);
        fputc((now->xx & 65280)>>8,fn);
        fputc(now->xx & 255,fn);
        fputc((now->yy & 65280)>>8,fn);
        fputc(now->yy & 255,fn);
        fputc(now->direction,fn);
        fputc(now->outputs,fn);
        fputc(now->inputs,fn);
        fputc(now->connection[0],fn);
        fputc(now->connection[1],fn);
        fputc(now->connection[2],fn);
        fputc(now->connection[3],fn);
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)                  /* handle the messages */
    {
        case WM_CREATE:
          ...
          ImageList_Add(g_hImageList,tb_edit,tb_edit_mask);//24
          
          ImageList_Add(g_hImageList,tb_blank,tb_blank_mask);//25

          ImageList_Add(g_hImageList,tb_thruster,tb_thruster_mask);//26
          ...
          tbb[id].iBitmap = MAKELONG(25,0);
          tbb[id].fsState = TBSTATE_ENABLED;
          tbb[id].fsStyle = TBSTYLE_CHECK;
          tbb[id].idCommand = CM_BLOCK_BLANK;
          
          id+=1;

          tbb[id].iBitmap = MAKELONG(26,0);
          tbb[id].fsState = TBSTATE_ENABLED;
          tbb[id].fsStyle = TBSTYLE_CHECK;
          tbb[id].idCommand = CM_BLOCK_THRUSTER;
          ...
          tbb[id].iBitmap = MAKELONG(24, 0);
          tbb[id].fsState = TBSTATE_ENABLED;
          tbb[id].fsStyle = TBSTYLE_CHECK;
          tbb[id].idCommand = CM_EDIT;
        ...
        case WM_TIMER:
          ...
          if (selectedmode==3)
          {
            if ((mouse_selected>=1) && (mouse_selected<=2))
            {
              draw_rectangle( (mouse_dragx*32)-view_left, (mouse_dragy*32)-view_top , (mouse_endx*32)-view_left, (mouse_endy*32)-view_top ,RGB(0,0,0),RGB(0,0,0),0);                     
            }
          }
          ...
          //draw_int(0, 20, keyboard_key[0] ,RGB(255,0,255));
          draw_int(0, 20, execsize ,RGB(255,0,255));
          //draw_int(0, 40, mouse_endx ,RGB(255,0,255));
          //draw_int(0, 60, floor(mousex/32) ,RGB(255,0,255));
          
           draw_int(0, 80, block_executed ,RGB(255,0,255));
          ...
          if (wParam==1001)
          {
            fps=frame;  
            frame=0; 
            block_executed=block_step_count; 
            block_step_count=0;             
          }
       ...
       case WM_COMMAND:
         ...
         switch(LOWORD(wParam))
         {
           ...
           case CM_EDIT:
             ...
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_CONNECT ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_DISCONNECT ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_EDIT ,  MAKELONG(1, 0) );
           ...
           case CM_BLOCK_BLANK:
             ...
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_RIGHT_SLIDER ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_LEFT_SLIDER ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_BLANK ,  MAKELONG(1, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_THRUSTER ,  MAKELONG(0, 0) );
             break; 
             
           case CM_BLOCK_THRUSTER:
                
             selectedblock=BTHRUSTER;
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_WIRE ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_NOT ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_DELTA ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_PUSH ,  MAKELONG(0, 0) );
             ...
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_LEFT_SLIDER ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_BLANK ,  MAKELONG(0, 0) );
             SendMessage(g_hToolBar, TB_CHECKBUTTON, CM_BLOCK_THRUSTER ,  MAKELONG(1, 0) );
             break; 
